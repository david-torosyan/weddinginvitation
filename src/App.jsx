...
export default function App() {
  ...
  const { couple, wedding, cover, hero, introduction, itinerary, events, notes, rsvp } = config;
  ...
  async function handleRsvp(event) {
    ...
    try {
      ...
      if (!guestCount || guestCount < 1) throw new Error(rsvp.invalidGuestCountMessage);
      submitRsvp({ guestName: form.get('guestName').trim(), guestCount, attendance: form.get('attendance'), submittedAt: new Date().toISOString() }).catch(() => {});
  ...
  return <div className="invite-shell">
    ...
    <main className={opened ? 'content visible' : 'content'}>
      ...
      <section className="welcome section-pad"><p className="section-kicker">{introduction.kicker}</p><h2>{introduction.heading}<br /><em>{introduction.emphasis}</em></h2>{introduction.paragraphs.map(paragraph => <p key={paragraph}>{paragraph}</p>)}<p className="date-line">{wedding.longDate}</p></section>
      <WeddingCalendar />
      <section className="itinerary section-pad" id="itinerary"><div className="section-intro"><p className="section-kicker">{itinerary.kicker}</p><h2>{itinerary.heading}<br /><em>{itinerary.emphasis}</em></h2><p>{itinerary.description}</p></div><div className="event-list">{events.filter(event => event.enabled !== false).map(event => <article className="event-card" key={event.id}><div className="event-image"><img src={event.image} alt={event.imageAlt || event.title} /><span>{event.number}</span></div><div className="event-copy"><div className="event-time"><Clock3 size={16} /> {event.time}</div><h3>{event.title}</h3><p className="event-venue">{event.venue}</p><p className="event-address"><MapPin size={15} /> {event.address}</p>{event.mapUrl && <a className="map-link" href={event.mapUrl} target="_blank" rel="noreferrer">Քարտեզը բացել <ArrowUpRight size={15} /></a>}</div></article>)}</div></section>
      <section className="timeline section-pad"><p className="section-kicker">{wedding.longDate}</p><h2>Անհամբեր սպասում ենք<br /><em>Ձեզ տեսնելուն</em></h2><Countdown /></section>
      <section className="details section-pad">{notes.items.map(note => <article key={note.title}>{note.icon === 'calendar' ? <CalendarDays size={22} /> : <Heart size={22} />}<h3>{note.title}</h3><p>{note.text}</p></article>)}</section>
      {rsvp.enabled && <section className="rsvp section-pad" id="rsvp"><p className="pretitle">{rsvp.kicker}</p><h2>{rsvp.heading}</h2><p>{rsvp.description}</p>{sent ? <div className="thanks">{rsvp.successMessage} <Heart size={17} fill="currentColor" /></div> : <form onSubmit={handleRsvp}><input name="guestName" required autoComplete="name" placeholder={rsvp.namePlaceholder} /><input name="guestCount" type="number" min="1" max="20" required inputMode="numeric" placeholder={rsvp.guestCountPlaceholder} /><select name="attendance" defaultValue="" required><option value="" disabled>{rsvp.attendancePlaceholder}</option>{rsvp.attendanceOptions.map(option => <option value={option.value} key={option.value}>{option.label}</option>)}</select><button disabled={busy} type="submit">{busy ? rsvp.sendingText : rsvp.submitButtonText} {!busy && <Send size={15} />}</button><p className="form-status" aria-live="polite">{status}</p></form>}</section>}
